...
#define REQ_TYPE_GET_PKT_RECEIPT    0x08
...
void MyMesh::recordGroupReceipt(const mesh::Packet* packet) {
  if (!packet) return;
  ...
  RecentFloodReceipt* slot = nullptr;
  uint32_t oldest = 0xFFFFFFFF;
  for (int i = 0; i < (int)(sizeof(recent_group_receipts) / sizeof(recent_group_receipts[0])); i++) {
    auto* entry = &recent_group_receipts[i];
    if (memcmp(entry->hash, hash, MAX_HASH_SIZE) == 0) {
      slot = entry;
      break;
    }
    if (entry->last_heard_at == 0) {
      slot = entry;
      break;
    }
    if (entry->last_heard_at < oldest) {
      oldest = entry->last_heard_at;
      slot = entry;
    }
  }
  ...
  if (memcmp(slot->hash, hash, MAX_HASH_SIZE) != 0) {
    memcpy(slot->hash, hash, MAX_HASH_SIZE);
    slot->heard_count = 0;
    slot->first_heard_at = now;
  }

  if (slot->heard_count < 0xFFFF) slot->heard_count++;
  slot->last_heard_at = now;
  slot->last_snr = packet->_snr;
}
...
int MyMesh::handleRequest(ClientInfo *sender, uint32_t sender_timestamp, uint8_t *payload, size_t payload_len) {
  ...
  } else if (payload[0] == REQ_TYPE_GET_PKT_RECEIPT && sender->isAdmin()) {
    if (payload_len >= 1 + MAX_HASH_SIZE) {
      uint16_t heard_count = 0;
      uint32_t heard_secs_ago = 0xFFFFFFFF;
      int8_t last_snr = 0;

      const uint8_t* want_hash = &payload[1];
      for (int i = 0; i < (int)(sizeof(recent_group_receipts) / sizeof(recent_group_receipts[0])); i++) {
        auto* entry = &recent_group_receipts[i];
        if (entry->last_heard_at == 0) continue;
        if (memcmp(entry->hash, want_hash, MAX_HASH_SIZE) == 0) {
          heard_count = entry->heard_count;
          heard_secs_ago = getRTCClock()->getCurrentTime() - entry->last_heard_at;
          last_snr = entry->last_snr;
          break;
        }
      }

      memcpy(&reply_data[4], &heard_count, 2);
      memcpy(&reply_data[6], &heard_secs_ago, 4);
      memcpy(&reply_data[10], &last_snr, 1);
      return 11;
    }
