...
void setup() {
  ...
  // --- DEBUG: check if persisted prefs exist ---
  Serial.printf("SPIFFS: /node_prefs %s\r\n",
                SPIFFS.exists("/node_prefs") ? "FOUND" : "NOT FOUND");
  ...
  Serial.printf(
      "Radio cfg (about to apply): freq=%.6f MHz, bw=%.2f kHz, sf=%d, cr=%d, tx=%d dBm\r\n",
      (double)the_mesh.getFreqPref(),
      (double)LORA_BW,
      (int)LORA_SF,
      (int)LORA_CR,
      (int)the_mesh.getTxPowerPref()
  );
